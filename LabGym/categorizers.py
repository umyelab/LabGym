...
class Categorizers:
    ...
    def rename_label(self, file_path, new_path, resize=None):
        folder_list = [
            i for i in os.listdir(file_path) if os.path.isdir(os.path.join(file_path, i))
        ]
        ...
        for folder in folder_list:
            name_list = [
                i for i in os.listdir(os.path.join(file_path, folder)) if i.endswith(".avi")
            ]

            if len(name_list) == 0:
                name_list = [
                    i for i in os.listdir(os.path.join(file_path, folder)) if i.endswith(".jpg")
                ]
                imagedata = True
                    ...
                    if previous_lenth is None:
                        ...
                        if previous_lenth != current_length:
                            ...
                            print(
                                ...
                                + str(i)
                                + ". Check the duration of animations!"
                            )
                        ...
                        if to_delete is not None and n in to_delete:
                            ...
                            if scale is not None:
                                ...
                                if width == 0:
                                    blob_scl = cv2.resize(
                                        blob,
                                        (blob.shape[1], int(blob.shape[0] * scale)),
                                        interpolation=cv2.INTER_AREA,
                                    )
                                else:
                                    blob_scl = cv2.resize(
                                ...
                                blob_black[y : y + blob_scl.shape[0], x : x + blob_scl.shape[1]] = (
                                    blob_scl
                                )
                                blob = blob_black
                ...
                if scale is not None:
                    ...
                    if width == 0:
                        pattern_image_scl = cv2.resize(
                            pattern_image,
                            (
                                pattern_image.shape[1],
                                int(pattern_image.shape[0] * scale),
                            ),
                            interpolation=cv2.INTER_AREA,
                        )
                    else:
                        pattern_image_scl = cv2.resize(
                            pattern_image,
                            (
                                int(pattern_image.shape[1] * scale),
                                pattern_image.shape[0],
                            ),
                            interpolation=cv2.INTER_AREA,
                        )
    ...
    def tres_block(self, x, filters, strides=2, block=False, basic=True):
        ...
        if basic is True:
            if block is False:
                shortcut = TimeDistributed(Conv2D(filters, (1, 1), strides=(strides, strides)))(
                    shortcut
                )
    ...
    def combined_network(
        ...
        if level_tconv < 5:
            animation_feature = self.simple_tvgg(
                animation_inputs,
                filters_tconv,
                level=level_tconv,
            ...
            animation_feature = self.simple_tresnet(
                animation_inputs,
                filters_tconv,
                level=level_tconv,
                with_classifier=False,
        ...
        if level_conv < 5:
            pattern_image_feature = self.simple_vgg(
                pattern_image_inputs,
                filters_conv,
                level=level_conv,
                with_classifier=False,
            )
        else:
            pattern_image_feature = self.simple_resnet(
                pattern_image_inputs,
                filters_conv,
                level=level_conv,
                with_classifier=False,
    ...
    def train_combnet(
        ...
        print(
            "Training Categorizer with both Animation Analyzer and Pattern Recognizer using the behavior examples in: "
            + str(data_path)
        )
        ...
        parameters = {
            "classnames": list(self.classnames),
            "dim_tconv": int(dim_tconv),
            "dim_conv": int(dim_conv),
            "channel": int(channel),
            "time_step": int(time_step),
            "network": 2,
            "level_tconv": int(level_tconv),
            "level_conv": int(level_conv),
            "inner_code": int(inner_code),
            "std": int(std),
            "background_free": int(background_code),
            "behavior_kind": int(behavior_mode),
        ...
        print("Start to augment validation examples...")
        if augvalid is True:
            test_animations, test_pattern_images, testY = self.build_data(
                test_files,
                dim_tconv=dim_tconv,
                dim_conv=dim_conv,
                channel=channel,
        ...
        model = self.combined_network(
            time_step=time_step,
            dim_tconv=dim_tconv,
            dim_conv=dim_conv,
            channel=channel,
            classes=len(self.classnames),
            level_tconv=level_tconv,
            level_conv=level_conv,
        )
        if len(self.classnames) == 2:
            model.compile(
                optimizer=SGD(learning_rate=1e-4, momentum=0.9),
        ...
        H = model.fit(
            [train_animations, train_pattern_images],
            trainY,
            batch_size=batch_size,
            validation_data=(
                [test_animations_tensor, test_pattern_images_tensor],
                testY_tensor,
            ),
            epochs=1000000,
            callbacks=[cp, es, rl],
    ...
    def test_categorizer(self, groundtruth_path, model_path, result_path=None):
        ...
        if len(incorrect_behaviors) == 0 and len(incorrect_classes) == 0:
                ...
                for i in filenames:
                    ...
                    if network != 1:
                        path_to_pattern_image = (
                            os.path.splitext(os.path.join(groundtruth_path, behavior, i))[0]
                            + ".jpg"
                        )
                        pattern_image = cv2.imread(path_to_pattern_image)
                        pattern_image = cv2.resize(
            ...
            print(
                classification_report(labels, predictions.argmax(axis=1), target_names=classnames)
            )
            report = classification_report(
                labels,
                predictions.argmax(axis=1),
            ...
            if result_path is not None:
                pd.DataFrame(report).transpose().to_excel(
                    os.path.join(result_path, "testing_reports.xlsx"),
                    float_format="%.2f",
                )
