    ...
    def acquire_information(self, background_free=True):
        ...
        while True:
            ...
            if time >= start_t:
                ...
                (contours, centers, heights, inners, blobs) = contour_frame(
                    frame,
                    self.animal_number,
                    background,
                    background_low,
        ...
        while True:
            ...
            if time >= start_t:
                ...
                if len(contours) == 0:
                    ...
                    self.pattern_images[0][frame_count_analyze] = np.array(
                        cv2.resize(
                            pattern_image,
                            (self.dim_conv, self.dim_conv),
                            interpolation=cv2.INTER_AREA,
        ...
        if self.categorize_behavior is True:
            ...
            for parameter_name in all_parameters:
                if parameter_name == "distance":
                    ...
                    individual_df.to_excel(
                        os.path.join(self.results_path, parameter_name + ".xlsx"),
                        float_format="%.2f",
                        index_label="time/ID",
                    )
            ...
            if len(summary) >= 1:
                pd.concat(summary, axis=1).to_excel(
                    os.path.join(self.results_path, "all_summary.xlsx"),
                    float_format="%.2f",
                    index_label="ID/parameter",
                )
