...
class AnalyzeAnimalDetector:
            ...
            for i in range(self.animal_number[animal_name]):
                ...
                if self.animation_analyzer is True:
                    ...
                    self.animations[animal_name][i] = [
                        np.zeros(
                            (self.length, self.dim_tconv, self.dim_tconv, self.channel),
                            dtype="uint8",
                        )
                    ] * self.total_analysis_framecount
                self.pattern_images[animal_name][i] = [
                        ...
                        if len(masks) == 0:
                            ...
                            exclusion_mask[
                                np.where(
                                    (
                                        np.sum(
                                            np.logical_and(masks[:, None], masks),
                                            axis=(2, 3),
                                        )
                                        / mask_area[:, None]
                                        > 0.8
            ...
            if time >= start_t:
                ...
                if writer is None:
                    ...
                    writer = cv2.VideoWriter(
                        os.path.join(self.results_path, "Annotated video.avi"),
    ...
    def export_results(self, normalize_distance=True, parameter_to_analyze=[]):
        ...
        self.analyze_parameters(
            ...
            parameter_to_analyze=parameter_to_analyze,
        )
                        ...
                        if parameter_name in [
                            ...
                            if parameter_name != "probability":
                                summary.append(
                                    ...
                                    .to_frame()
                                    .reset_index(drop=True)
                                    .rename(columns={0: parameter_name + "_mean"})
                                )
                                summary.append(
                                    individual_df.max(axis=1, skipna=True)
                                    .to_frame()
                                    .reset_index(drop=True)
                                    .rename(columns={0: parameter_name + "_max"})
                                )
                                summary.append(
    ...
    def generate_data(self, background_free=True, skip_redundant=1):
        ...
        for animal_name in self.animal_kinds:
            for i in range(self.animal_number[animal_name]):
                os.makedirs(
                    os.path.join(self.results_path, str(animal_name) + "_" + str(i)),
                    exist_ok=True,
                )
    ...
    def analyze_images_individuals(
        self,
        path_to_detector,
        path_to_images,
        results_path,
        ...
        for path_to_image in list(path_to_images):
                ...
                for behavior_name in names_and_colors:
                    for animal_name in animal_kinds:
                        if animal_name in animal_to_include:
                            ...
                            animal_information[behavior_name][animal_name]["time"][image_name] = (pd.to_datetime(int(os.path.getmtime(path_to_image)), utc=True, unit='s')).strftime('%H:%M:%S')
                            animal_information[behavior_name][animal_name]["date"][image_name] = (pd.to_datetime(int(os.path.getmtime(path_to_image)), utc=True, unit='s')).strftime('%Y-%m-%d')
            ...
            if generate is False:
                cv2.imwrite(
                    os.path.join(results_path, "Annotated_" + image_name + ".jpg"),
                    image,
                )
